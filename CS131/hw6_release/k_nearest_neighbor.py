...
def compute_distances(X1, X2):
    ...
    X1_suqare = np.sum(X1**2,axis=1,keepdims=True)
    X2_square = np.sum(X2**2,axis=1,keepdims=True).T
    X1dotX2 = X1 @ X2.T # [M,N]
    dists = X1_suqare - 2*X1dotX2 + X2_square
    pass
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    # YOUR CODE HERE
    pass
    index = np.argsort(dists,axis=1)[:,:k]
    import pdb
    # pdb.set_trace()
    for i in range(num_test):
        result = y_train[index[i]]
...
def split_folds(X_train, y_train, num_folds):
    ...
    for j in range(num_folds):
        ...
        X_vals[j] = X_train_folds[j]
        y_vals[j] = y_train_folds[j]
        
        X_trains[j] = np.vstack(X_train_folds[:j]+X_train_folds[j+1:])
#         pdb.set_trace()
        y_trains[j] = np.hstack(y_train_folds[:j]+y_train_folds[j+1:])
    # END YOUR CODE
