...
def sliding_window(image, template_feature, step_size, window_size, pixel_per_cell=8,
    ...
    pad_image = np.lib.pad(
        ...
        ((winH // 2,
         ...
         (winW // 2,
          winW - winW // 2)),
        mode='constant')#前后各padding winH的一部分
    ...
    for r in range(0, H, step_size):
        for c in range(0, W, step_size):
            ...
            hog_feature =  feature.hog(window,pixels_per_cell=(pixel_per_cell,pixel_per_cell),block_norm='L1',visualize=False)
            score = np.sum(np.dot(hog_feature,template_feature))
            if score > max_score:
                max_score,maxr,maxc = score,r,c
            ### END YOUR CODE
...
def pyramid_score(image, template_feature, shape, step_size=20,
    ...
    for i,result in enumerate(images):
        (scale, image) = result
        score,r,c,response_map = sliding_window(image,template_feature,step_size=step_size,window_size=shape,pixel_per_cell=pixel_per_cell,return_unresized_response=False)
        if score>max_score:
            max_score, maxr, maxc, max_scale, max_response_map = score, r, c, scale, response_map
...
def compute_displacement(part_centers, face_shape):
    ...
    ### YOUR CODE HERE
    pass
    d = np.array(face_shape)/2 - part_centers
    mu = np.mean(d,axis=0)
