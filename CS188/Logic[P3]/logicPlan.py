...
"""
In logicPlan.py, you will implement Logic[P3] planning methods which are called by
...
def findModel(sentence: Expr) -> Dict[Expr, bool]:
    """Given a propositional Logic[P3] sentence (i.e. a Expr instance), returns a satisfying
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    pl_true may be useful here; see Logic[P3].py for its description.
    """
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    Expr instance in CNF (conjunctive normal form) that represents the Logic[P3]
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    CNF (conjunctive normal form) that represents the Logic[P3] that at most one of
...
def exactlyOne(literals: List[Expr]) -> Expr:
    """
    Given a list of Expr literals, return a single Expr instance in 
    CNF (conjunctive normal form)that represents the Logic[P3] that exactly one of
    the expressions in the list is true.
    """
    "*** BEGIN YOUR CODE HERE ***"
    util.raiseNotDefined()
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    Given:
        ...
        successorAxioms(t, walls_grid, non_outer_wall_coords) -> Expr: function that generates
            the sensor model axioms. If None, it's not provided, so shouldn't be run.
    Return a Logic[P3] sentence containing all of the following:
...
mp = mapping
flp = foodLogicPlan
# Sometimes the Logic[P3] module uses pretty deep recursion on long expressions
...
def extractActionSequence(model: Dict[Expr, bool], actions: List) -> List:
    ...
    model: Propositional Logic[P3] model stored as a dictionary with keys being
