...
class PropSymbolExpr(Expr):
    ...
    def __init__(self, sym_str: str, *index: Tuple[int], time: int = None):
        """Constructor taking a propositional Logic[P3] symbol name and an optional set of index values,
...
def expr(s):
    """Create an Expr representing a Logic[P3] expression by parsing the input
...
def is_var_symbol(s):
    "A Logic[P3] variable symbol is an initial-lowercase string."
    return is_symbol(s) and s[0].islower()

def is_prop_symbol(s):
    """A proposition Logic[P3] symbol is an initial-uppercase string other than
    TRUE or FALSE."""
...
def pl_true(exp, model={}):
    """Return True if the propositional Logic[P3] expression is true in the model,
    ...
    if op == '<=>':
        ...
        raise ValueError("illegal operator in Logic[P3] expression" + str(exp))
