...
#include"vm.h"
extern void __dummy();
extern pagetable_t swapper_pg_dir;
...
void task_init() {
	...
	for(int i=1; i<NR_TASKS; i++)
	{
		...
		task[i]->thread.sstatus = SSTATUS_SUM | SSTATUS_SPP | SSTATUS_SPIE;
		
		task[i]->pgd = (pagetable_t)kalloc();
		task[i]->kernel_pgd = swapper_pg_dir;
		
		create_mapping(task[i]->pgd,USER_START, uapp_start,uapp_end-uapp_start,PTE_X | PTE_R | PTE_W | PTE_V);
		//TODO: how to copy the swapper_pg_dir into process PageTable?
