...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pass
        row = torch.arange(0,max_len).unsqueeze(1)
        col = 10000**(-torch.arange(0,embed_dim,2)/embed_dim)
        pe[:,:,::2] = torch.sin(row*col)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # assert S != T
        H = self.num_heads
        Q,K,V = self.query(query),self.key(key),self.value(value)   # 得到输入的 Q,K,V
        Q,K,V = Q.view(N,S,H,-1).transpose(1,2),K.view(N,T,H,-1).transpose(1,2),V.view(N,T,H,-1).transpose(1,2)
        # head 在第二维,
        # 应为每个单词的head要一一对应:第一个head只能和第一个head计算attention
        ####  要分两部,先切割head,再把head和单词个数维度互换
        ...
        A = Q @ K.transpose(2,3)  # [N,H,S,T]: A_n,h,s,t表示source中第n个句子的s个单词的第h个head和t
                                  # arget中第n个句子的第t个单词的第h个head的关系
        A = A /math.sqrt(D / H)
        if attn_mask is not None:
            A = A.masked_fill(attn_mask == False, -1e32)
        A = self.softmax(A)
        A = self.dropout(A)
        output = A @ V  # [N,H,S,E//H]
        import pdb
        # pdb.set_trace()
        output = output.transpose(1,2).reshape(N,S,-1)  # [N,S,D]
        output = self.proj(output)  # 表示啥意思呢 ?
