...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if k==i :
                continue
            denominator_i += torch.exp(sim(z_k,out[i]) / tau)
        for i in range(2*N):
            if k+N==i :
                continue
            denominator_j += torch.exp(sim(z_k_N,out[i]) / tau)
        loss_i = -torch.log(numerator/denominator_i)
        loss_j = -torch.log(numerator/denominator_j)
...
def compute_sim_matrix(out):
    ...
    pass
    out_norm = torch.linalg.norm(out,dim=1,keepdim=True) # N X 1
    sim_norm = out_norm * out_norm.T
    sim_matrix = out @ out.T / sim_norm
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator =torch.exp( torch.cat([pos_pairs,pos_pairs],dim=0).to(device) / tau)
