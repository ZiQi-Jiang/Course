...
def rnn_backward(dh, cache):
    ...
    dx,dWx,db,dWh,dh0,dh_ = np.zeros((N,T,D)),np.zeros((D,H)), np.zeros(H),np.zeros((H,H)), np.zeros((N,H)), np.zeros((N,H))
    for i in range(T-1, -1, -1):
        # if i!=0:
            import pdb
            dx_, dh_, dWx_, dWh_, db_ = rnn_step_backward(dh[:,i,:] + dh_, cache[i])
            db += db_
            dWh += dWh_
            dWx += dWx_
            dx[:,i,:] = dx_
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x,prev_h,prev_c,Wh,Wx,b,next_c,next_h,i,f,o,g)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # ct = f⊙ct−1 + i⊙g
    # ht = o⊙tanh(ct)
    (x,prev_h,prev_c,Wh,Wx,b,next_c,next_h,i,f,o,g) = cache
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        step_dx, step_dprev_h, step_dprev_c, step_dWx, step_dWh, step_db = lstm_step_backward(dh[:,t,:]+dnext_h, dnext_c, cache[t])
        dx.append(step_dx)
