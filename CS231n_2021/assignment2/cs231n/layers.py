...
def svm_loss(x, y):
    ...
    pass
    N = x.shape[0]
    groud_truth = np.expand_dims(x[range(N), y], axis=1)

    margin = x - groud_truth + 1
    margin[range(N), y] = 0
    loss = np.sum(margin)
...
def batchnorm_backward(dout, cache):
    ...
    pass
    # the code is implemented accroding to the original paper
    x, x_hat, mean, var, gamma, beta, eps = cache
    N = x.shape[0]
    dbeta = np.sum(dout, axis=0)  # D X 1

    dgamma = np.sum(dout * x_hat, axis=0)  # D X 1

    dx_hat = dout * gamma  # N X D

    dvar = np.sum(dx_hat * (x - mean) * -0.5 * (var + eps) ** -1.5, axis=0)  # D X 1
    dmean = np.sum(dx_hat * (-1) * (var + eps) ** -0.5, axis=0) + dvar * (1 / N) * np.sum(-2 * (x - mean),
                                                                                          axis=0)  # D X 1

    dx = dx_hat * (var + eps) ** -0.5 + dvar * (2 / N) * (x - mean) + dmean / N  # N * D
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    if mode == "train":
        ...
        raise ValueError('Invalid forward layernorm mode "%s"' % mode)
...
def layernorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat * (x - mean) * -0.5 * (var + eps) ** -1.5, axis=1, keepdims=True)  # D X 1
    dmean = np.sum(dx_hat * (-1) * (var + eps) ** -0.5, axis=1,keepdims=True) + dvar * (1 / D) * np.sum(-2 * (x - mean),
                                                                                          axis=1,keepdims=True)  # D X 1

    import pdb
    pdb.set_trace()
    dx = dx_hat * (var + eps) ** -0.5 + dvar * (2 / D) * (x - mean) + dmean / D  # N * D
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x_tmp = x
    x = np.pad(x, ((0, 0), (0, 0), (P, P), (P, P)), 'constant', constant_values=(0, 0))
    # print(x.shape)
    Hout = int((H + 2 * P - HH) / S + 1)
    Wout = int((W1 + 2 * P - WW) / S + 1)
    ...
    for n in range(N):
        for f in range(F):
            for h in range(Hout):
                for W in range(Wout):
                    kernel = w[f]  # [ C, HH, WW ]
                    area = x[n, :, h * S:h * S + HH, W * S:W * S + WW]  # [ C, HH, WW ]
                    out[n, f, h, W] = np.sum(area * kernel) + b[f]

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x_tmp, w, b, conv_param)
    return out, cache
        ...
        for f in range(F):
            for h in range(Hout):
                for W in range(Wout):
                    ...
                    area = x[n, :, h * S:h * S + HH, W * S:W * S + WW]  # [ C, HH, WW ]
                    # out[n, f, h, W] = np.sum(area * kernel) + b[f]
                    dw[f] += area * dout[n, f, h, W]
                    import pdb
                    if area.shape != kernel.shape:
                        pdb.set_trace()
                    dx[n, :, h * S:h * S + HH, W * S:W * S + WW] += kernel * dout[n, f, h, W]
        ...
        for c in range(C):
            for h in range(HH):
                for w in range(WW):
                    ...
                    res = (x[n, c, h * stride:h * stride + pool_height, w * stride:w * stride + pool_width]==np.max(x[n, c, h * stride:h * stride + pool_height, w * stride:w * stride + pool_width])).astype('float')
                    dx[n, c, h * stride:h * stride + pool_height, w * stride:w * stride + pool_width] = res*dout[n,c,h,w]
